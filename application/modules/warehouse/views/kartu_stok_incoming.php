...
<div class="box box-primary">
    ...
    <div class="box-body">
        <div class="table-responsive">
            <table id="table-kartu-stok-incoming" class="table table-bordered table-striped">
                <thead class="bg-blue">
                    <tr>
                        <th rowspan='2'>#</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#btnFilter').on('click', function(e) {
            e.preventDefault();
            if ($.fn.dataTable.isDataTable('#table-kartu-stok-incoming')) {
                $('#table-kartu-stok-incoming').DataTable().ajax.reload(null, true);
            }
        });
        ...
        $('#btnExport').on('click', function(e) {
            ...
            window.location.href = base_url + active_controller + 'export_excel_kartu_stok_incoming?start_date=' + start + '&end_date=' + end;
        });
    });

    function DataTables(status = null) {
        var dataTable = $('#table-kartu-stok-incoming').DataTable({
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "autoWidth": false,
            ...
            "ajax": {
                url: base_url + active_controller + 'data_side_kartu_stok_incoming',
                type: "post",
                data: function(d) {
                    d.status = status;
